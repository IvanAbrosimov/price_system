...
const ProductsService = {
  ...
  getByManufacturer: vi.fn((manufacturer: string) => 
    Promise.resolve(mockProducts.filter(p => p.manufacturer === manufacturer))
  ),
  getByArticle: vi.fn((article: string) => 
    Promise.resolve(mockProducts.find(p => p.article === article.toLowerCase()) || null)
  ),
  getStock: vi.fn((article: string) => {
  ...
  getManufacturers: vi.fn(() => 
    Promise.resolve([...new Set(mockProducts.map(p => p.manufacturer))])
  )
}
