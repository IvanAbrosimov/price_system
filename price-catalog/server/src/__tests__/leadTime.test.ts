 ...
 */
export function getDynamicLeadTime(astanaQty: number, almatyQty: number, orderQty: number): string {
  if (orderQty <= 0) return ''
  if (astanaQty >= orderQty) return '6-10 дней'
  if ((astanaQty + almatyQty) >= orderQty) return '10-14 дней'
  return 'по запросу'
}
